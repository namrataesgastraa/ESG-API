...
const getFirstSheetRows = (workbook, names) => {
...
const parseGraphs = (workbook) => {
  const sheetName = workbook.SheetNames.find(
    (name) => normalize(name) === normalize("Graphical Insights")
  );
  if (!sheetName) return [];
  ...
  const headerIndex = aoa.findIndex((row) =>
    Array.isArray(row) && row.some((cell) => normalize(cell) === normalize("Chart Title"))
  );
  if (headerIndex === -1) return [];

  const headers = aoa[headerIndex].map((cell) => clean(cell));
  const columnOf = (label) => headers.findIndex((header) => normalize(header) === normalize(label));

  const cols = {
    type: columnOf("Chart Type"),
    category: columnOf("Metric Category"),
    title: columnOf("Chart Title"),
    label: columnOf("Data Point Label"),
    value: columnOf("Value"),
    unit: columnOf("Unit"),
    benchmarkLabel: columnOf("Benchmark Label"),
    benchmarkValue: columnOf("Benchmark Value"),
  ...
  for (let r = headerIndex + 1; r < aoa.length; r += 1) {
    ...
    const type = cell(row, cols.type) || lastType;
    const category = cell(row, cols.category) || lastCategory;
    const title = cell(row, cols.title) || lastTitle;
    const source = cell(row, cols.source) || lastSource;
    lastType = type;
    ...
    if (!chart) {
      ...
      byTitle.set(title, chart);
      charts.push(chart);
    }

    chart.points.push({
      label,
      value: toNumber(cell(row, cols.value)),
      unit: cell(row, cols.unit),
      benchmark_label: cell(row, cols.benchmarkLabel),
      benchmark_value: toNumber(cell(row, cols.benchmarkValue)),
    });
  }

  return charts.filter((chart) => chart.points.length > 0);
};

exports.parseWhitePaperExcel = (buffer) => {
  ...
  const related_whitepapers = parseRelated(
    getFirstSheetRows(workbook, ["Related Whitepapers", "Related Case Studies"])
  );
  const graphs = parseGraphs(workbook);

  const industryTag = clean(info["Industry Tag"]) || "Whitepaper";

  return {
    title: clean(info["Title"]) || "Untitled Whitepaper",
    slug:
      clean(info["Slug"]) ||
      clean(info["URL Slug"]) ||
      clean(info["SEO Slug"]) ||
    ...
    author_name: clean(info["Author"]) || clean(info["Client"]),
    report_type: clean(info["Report Type"]),
    pages: clean(info["Pages"]),
    frameworks_covered: toList(info["Frameworks Covered"]),
    ...
    graphical_enabled: toBoolean(info["Graphical Enabled"]),
